    ...
    const handleOk = async () => {
        try {
            ...
            if (index !== -1) {
                ...
                getZksEra(values.address).then(({balance2, tx2, usdcBalance, zatBalance}) => {
                    updatedData[index] = {
                        ...updatedData[index],
                        zks2_balance: balance2,
                        zks2_tx_amount: tx2,
                        zks2_usdcBalance: usdcBalance,
                        zks2_zatBalance: zatBalance
                    };
                    setData(updatedData);
                ...
                getZksEra(values.address).then(({balance2, tx2, usdcBalance, zatBalance}) => {
                    ...
                    newEntry.zks2_zatBalance = zatBalance;
                    setData([...newData]);
        ...
        try {
            ...
            for (let key of selectedKeys) {
                ...
                if (index !== -1) {
                    ...
                    setData([...newData]);
                    getZksEra(item.address).then(({balance2, tx2, usdcBalance, zatBalance}) => {
                        item.zks2_balance = balance2;
                        item.zks2_tx_amount = tx2;
                        item.zks2_usdcBalance = usdcBalance;
                        item.zks2_zatBalance = zatBalance;
                        setData([...newData]);
                        localStorage.setItem('addresses', JSON.stringify(data));
                    });
        ...
        try {
            ...
            for (let address of addresses) {
                ...
                if (index !== -1) {
                    ...
                    getZksEra(address).then(({balance2, tx2, usdcBalance, zatBalance}) => {
                        updatedData[index] = {
                            ...updatedData[index],
                            zks2_balance: balance2,
                            zks2_tx_amount: tx2,
            ...
            <Content>
                    ...
                    >
                        ...
                        <ColumnGroup title="zkSyncEra" className={"zks_era"}>
                            ...
                            <Column title="USDC" dataIndex="zks2_usdcBalance" key="zks2_usdcBalance" align={"center"}
                                    render={(text, record) => (text === null ? <Spin/> : text)}/>
                            <Column title="ZAT" dataIndex="zks2_zatBalance" key="zks2_zatBalance" align={"center"}
                                    render={(text, record) => (text === null ? <Spin/> : text)}/>
                ...
                <Card>
                    <div style={{width: '100%', display: 'flex', justifyContent: 'space-between', gap: '10px'}}>
                        <Button type="primary" onClick={showModal} size={"large"} style={{width: "20vw"}}
                                icon={<PlusOutlined/>}>
                            添加地址
                        </Button>
                        <Button type="primary" onClick={showBatchModal} size={"large"} style={{width: "20vw"}}
                                icon={<UploadOutlined/>}>
                            批量添加地址
                        </Button>
                        <Button type="primary" onClick={handleRefresh} loading={isLoading} size={"large"}
                                style={{width: "20vw"}} disabled={!selectedKeys.length} icon={<SyncOutlined/>}>
                            刷新选中地址
                        </Button>
                        <Popconfirm title={"确认删除？"} onConfirm={handleDeleteSelected} disabled={!selectedKeys.length}>
                            <Button 
                                type="primary" 
                                danger size={"large"} 
                                style={{width: "20vw"}}
